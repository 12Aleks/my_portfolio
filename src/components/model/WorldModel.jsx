...
export default function WorldModel(props) {
    const group = useRef()
    const { nodes, materials, animations } = useGLTF('/models/world-transformed.glb')
    const { actions } = useAnimations(animations, group)

    // useEffect(() => {
    //     if (actions) {
    //         if (actions && Object.keys(actions).length > 0) {
    //             const firstAnimation = Object.keys(actions)[0]; // Get the first animation name
    //             actions[firstAnimation]?.play(); // Play the animation
    //         }
    //     }
    // }, [actions]);
