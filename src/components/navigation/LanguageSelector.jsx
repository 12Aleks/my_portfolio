    ...
    return (
        <div className="relative">
            <button
                ...
                className="flex items-center px-3 py-2 bg-neutral-700 rounded-sm text-sm focus:outline-none"
            >
                {currentLocale.toUpperCase()}
                <ChevronDown className={`ml-2 w-4 h-4 transition-transform ${isDropdownOpen ? "rotate-180" : "rotate-0"}`} />
            </button>
            {isDropdownOpen && (
                <div className="absolute right-0 mt-2 w-16 bg-white text-black rounded-sm shadow-lg z-50 overflow-hidden">
                    {languages.map((lang) => (
                        <button
                            key={lang.code}
                            onClick={() => {
