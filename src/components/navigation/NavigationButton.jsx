...
const getIcon = iconName => {
    switch (iconName) {
        ...
        case "projects":
            return <FileStack className="w-full h-auto scale-x-[-1]" strokeWidth={1.5}/>;
        case "contact":
            return <Mail className="w-full h-auto" strokeWidth={1.5}/>;
        case "linkedin":
            return <Linkedin className="w-full h-auto scale-x-[-1]" strokeWidth={1.5}/>;
        case "github":
            return <Github className="w-full h-auto" strokeWidth={1.5}/>;
        case "facebook":
            return <Facebook className="w-full h-auto scale-x-[-1]" strokeWidth={1.5}/>;
        case "resume":
...
const NavigationButton = ({ x, y, z, label, link, icon, newTab, rotation, setIsHovered }) => {
        ...
        >
            ...
            >
                ...
                >
                    ...
                    top-1/2 -translate-y-1 bg-background text-sm rounded-md shadow-lg whitespace-nowrap scale-x-[-1]">
                        {label}
                    </span>
                </span>
            </Link>
